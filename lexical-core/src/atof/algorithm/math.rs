...
pub fn trailing_zeros(x: &[Limb]) -> usize {
    ...
    let mut count = index.saturating_mul(<Limb as num::Integer>::BITS);
...
pub fn bit_length(x: &[Limb]) -> usize {
    ...
    <Limb as num::Integer>::BITS.checked_mul(x.len())
        .map(|v| v - nlz)
    ...
    where T: CloneableVecLike<Limb>
{
    ...
    let bits = <Limb as num::Integer>::BITS;
    ...
    where T: CloneableVecLike<Limb>
{
    ...
    let mut r = T::default();
    r.reserve_exact(n);
    let rs = <Limb as num::Integer>::BITS - s;
    for i in 0..n-1 {
